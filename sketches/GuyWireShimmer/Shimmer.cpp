...
float medium_champagne_g = 120/255.0;
float medium_champagne_b = 30/255.0;
...
void Shimmer::reset() {
    ...
    //Shimmer initiation
  for (uint16_t i = BLANK_AT_FRONT; i < LED_COUNT; i++)
  {
    value[i] = 0.0;
    maxValue[i] = random(ledMax);
    death[i] = ledMax + maxValue[i] + (stepSize / 2) + (0.5 * random(ledMax));
    direction[i] = 1;
  }
...
void Shimmer::draw(CRGB* leds) { 
  ...
  for (int i = BLANK_AT_FRONT; i < LED_COUNT; i++) {
